...
function AppInnerContent() {
  ...
  const {
    song, structure, updateState, updateSongAndStructureWithHistory,
  ...
  const {
    isGenerating,
    selectedLineId, setSelectedLineId, suggestions, isSuggesting, generateSong,
    generateSuggestions, applySuggestion, clearSelection,
  } = useComposerContext();
  ...
  const { index: webSimilarityIndex, triggerNow: triggerWebSimilarity, resetIndex: resetWebSimilarityIndex } =
    useSimilarityEngine(song, title, songLanguage);

  // ── Derived state ─────────────────────────────────────────────────────────
  const { hasRealLyricContent, hasExistingWork, webBadgeLabel } = useDerivedAppState({
    song, structure, rhymeScheme, topic, mood, isMarkupMode, markupText, webSimilarityIndex,
  });
  ...
  const {
    ...
    setApiErrorModal, setConfirmModal, setActiveTab, setIsLeftPanelOpen, setIsStructureOpen,
  ...
  const {
    handleSaveToLibrary, handleLoadLibraryAsset, handleDeleteLibraryAsset,
    handlePurgeLibrary, handleOpenSaveToLibraryModal,
  } = useLibraryActions({
