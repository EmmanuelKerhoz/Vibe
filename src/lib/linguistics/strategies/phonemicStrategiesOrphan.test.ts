...
describe('AustronesianStrategy (ALGO-AUS)', () => {
  ...
  it('ID: "menulis" strips "men" prefix → stem "ulis" used for rime', () => {
    // "menulis" → longest matching prefix "men" stripped → stem "ulis"
    // stem "ulis": u(open) + lis → last open syllable "u" → nucleus 'u'
    const r = PhonologicalRegistry.analyze('menulis', 'id');
    expect(r?.rhymeNucleus.nucleus).toBe('u');
  });

  it('ID: compare("membeli", "duduk") → score < 0.5 (i vs u nucleus mismatch)', () => {
    // "membeli" → strips "mem" → "beli" → last open syllable "li" → nucleus 'i'
    // "duduk"  → last open syllable "du" → nucleus 'u'
    const r = PhonologicalRegistry.compare('membeli', 'duduk', 'id');
    expect(r?.score).toBeLessThan(0.5);
  });
