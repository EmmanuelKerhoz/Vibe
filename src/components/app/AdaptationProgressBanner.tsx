...
function StepIndicatorRow({
  ...
  activeStepId: typeof ORDERED_STEP_IDS[number];
  isDone: boolean;
  isFailed: boolean;
}) {
  ...
  const activeIdx = ORDERED_STEP_IDS.indexOf(activeStepId);
...
export function AdaptationProgressBanner({
  ...
  const effectiveActiveStepId = (isFailed
    ? 'reviewing'
  ...
  const banner = (
    ...
    >
      ...
      <StepIndicatorRow
        activeStepId={effectiveActiveStepId}
        isDone={isDone}
        isFailed={isFailed}
      />
          ...
          {result.warnings.length > 0 && (
            <ul className="flex flex-col gap-0.5 list-none pl-0 mt-0.5">
              {result.warnings.map((warning, index) => (
                <li key={`${warning}-${index}`} className="flex items-start gap-1 text-amber-300/80">
