...
export function ViewModeSelector({ editMode, switchEditMode, disabled }: ViewModeSelectorProps) {
  ...
  return (
    <div className="flex items-center gap-1.5 shrink-0">
      <Tooltip title={t.editor.lyricsEditors ?? 'Editor view'} positioning={{ position: 'below', align: 'start' }}>
        <div style={{ minWidth: '110px', maxWidth: '150px' }}>
          <LcarsSelect
            value={editMode}
            onChange={(value) => switchEditMode(value as EditMode)}
            triggerLabel={<span className="flex items-center gap-1.5 text-[11px] uppercase">{t.editor.lyricsEditors ?? 'View'}</span>}
            options={[
