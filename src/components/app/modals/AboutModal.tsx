      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl max-h-[90vh] flex flex-col bg-[var(--bg-card)] border border-[var(--border-color)] rounded-[24px_8px_24px_8px] shadow-2xl overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              ...
              <div className="p-8 text-center space-y-6">
                ...
                {/* Tech Info */}
                <div className="grid grid-cols-1 gap-4 pt-4 border-t border-[var(--border-color)] sm:grid-cols-2">
                  <div className="flex min-w-0 flex-col items-center gap-1">
                    <span className="micro-label text-[var(--text-secondary)]">{t.about.engineLabel}</span>
                    <span className="text-xs text-[var(--text-secondary)] telemetry-text">{t.about.engine}</span>
                  </div>
                  <div className="flex min-w-0 flex-col items-center gap-1">
                    <span className="micro-label text-[var(--text-secondary)]">{t.about.modelLabel}</span>
                    <span className="text-xs text-[var(--text-secondary)] telemetry-text break-all">{AI_MODEL_NAME}</span>
                  </div>
                  <div className="flex min-w-0 flex-col items-center gap-1">
                    <span className="micro-label text-[var(--text-secondary)]">{t.about.apiKeyLabel}</span>
                    <span className="text-xs text-[var(--text-secondary)] telemetry-text break-all">{AI_KEY_ENV_VAR}</span>
                  </div>
                  <div className="flex min-w-0 flex-col items-center gap-1">
                    <span className="micro-label text-[var(--text-secondary)]">{t.about.licenseLabel}</span>
                    <span className="text-xs text-[var(--text-secondary)] telemetry-text">{t.about.license}</span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 pt-4">
                  ...
                  <a
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-zinc-500/10 hover:bg-zinc-500/20 border border-zinc-500/20 hover:border-zinc-500/40 text-[var(--text-primary)] hover:text-[var(--text-primary)] rounded-lg transition-all text-xs font-medium"
                  >
