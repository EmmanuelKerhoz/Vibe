          ...
          <div className="p-6 space-y-3">
            <div className="flex items-center justify-between gap-3">
              ...
              >
                ...
                {t.similarity.webRefresh}
              </button>
            </div>

            {status === 'idle' && (
              <div className="px-4 py-3 rounded-lg bg-[var(--bg-app)] border border-[var(--border-color)] text-sm text-[var(--text-secondary)]">
                {t.similarity.webIdle}
              </div>
            )}
                  ...
                  >
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      <div className="flex items-center gap-2 min-w-0">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-bold border ${scoreBadgeClass(c.score)}`}
                        >
                          {c.score}%
                        </span>
                        <span
                          className="text-sm font-medium text-[var(--text-primary)] truncate max-w-[280px]"
                          title={c.title}
                        >
                          {c.title}
                        </span>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {c.url && (
                          <a
                            ...
                            className="text-[var(--accent-color)] hover:opacity-70 transition-opacity"
                            title="Open source"
                          >
                    ...
                    <p className="text-[10px] text-[var(--text-secondary)] italic">
                      {scoreLabel(c.score)} similarity · {c.matchedSegments.length} matched
                      segment{c.matchedSegments.length !== 1 ? 's' : ''}
                    </p>
                  </div>
                ))}
              </div>
                  ...
                  >
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      <div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
                          <span className="px-2 py-0.5 bg-[var(--bg-card)] border border-[var(--border-color)] rounded text-[var(--text-secondary)] font-mono">
                            {match.versionName}
                          </span>
                    ...
                    <div className="grid grid-cols-3 gap-3 mt-4">
                      <div className="rounded-lg bg-[var(--bg-card)] border border-[var(--border-color)] p-3">
                        <div className="text-[10px] uppercase tracking-widest text-[var(--text-secondary)] mb-1">
                          {t.similarity.sharedWords}
                        </div>
                        <div className="text-xl font-light text-[var(--text-primary)] font-mono">
                          {match.sharedWords}
                        </div>
                      </div>
                      <div className="rounded-lg bg-[var(--bg-card)] border border-[var(--border-color)] p-3">
                        <div className="text-[10px] uppercase tracking-widest text-[var(--text-secondary)] mb-1">
                          {t.similarity.sharedLines}
                        </div>
                        <div className="text-xl font-light text-[var(--text-primary)] font-mono">
                          {match.sharedLines}
                        </div>
                      ...
                      <div className="rounded-lg bg-[var(--accent-color)]/10 border border-[var(--accent-color)]/20 p-3">
                        <div className="text-[10px] uppercase tracking-widest text-[var(--text-secondary)] mb-1">
                          {t.similarity.matchedSections}
                        </div>
                        <div className="text-xl font-light text-[var(--text-primary)] font-mono">
                          {match.matchedSections.length}
                        </div>
                      </div>
                    </div>
                      ...
                      {match.sharedKeywords.length > 0 && (
                        <div>
                          <div className="text-[10px] uppercase tracking-widest text-[var(--text-secondary)] mb-2">
                            {t.similarity.sharedKeywords}
                          </div>
                          <div className="flex flex-wrap gap-1.5">
                            {match.sharedKeywords.map(keyword => (
                              <span
                                key={keyword}
                                className="px-2.5 py-1 text-xs bg-[var(--bg-card)] border border-[var(--border-color)] rounded text-[var(--text-primary)] hover:border-[var(--accent-color)]/30 transition-colors"
                              >
                                {keyword}
                              </span>
                            ))}
