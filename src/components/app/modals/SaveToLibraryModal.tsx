...
export function SaveToLibraryModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg flex flex-col animate-in zoom-in-95 duration-300 glass-panel border border-white/10 rounded-[24px_8px_24px_8px] shadow-2xl overflow-hidden dark:border-white/8">
        {/* Header */}
        ...
        <div className="p-6 border-b border-[var(--border-color)]">
          <div className="flex items-center justify-between gap-4 p-4 rounded-[12px_4px_12px_4px] bg-[var(--accent-color)]/10 border border-[var(--accent-color)]/20">
            ...
            <button
              onClick={onSave}
              disabled={isSaving}
              className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-[var(--accent-color)] hover:opacity-90 text-[var(--on-accent-color)] text-xs font-bold rounded-lg transition-all disabled:opacity-60 disabled:cursor-not-allowed shadow-sm"
            >
              {isSaving ? (
          ...
          {libraryAssets.length === 0 ? (
            ...
            <div className="px-6 pb-6 pt-2 space-y-2">
              {[...libraryAssets].reverse().map(asset => (
                <div
                  key={asset.id}
                  className="flex items-center gap-3 p-3 rounded-lg bg-[var(--bg-app)] border border-[var(--border-color)] hover:border-[var(--accent-color)]/30 transition-colors"
                >
